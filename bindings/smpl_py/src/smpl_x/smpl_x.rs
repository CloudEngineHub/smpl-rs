...
impl PySmplX {
    #[staticmethod]
    #[pyo3(signature = (path, pygender, max_num_betas=None, max_expression_blendshapes=None))]
    #[pyo3(
    ...
    pub fn get_for_candle(&self) -> PySmplXGPU {
        match &self.inner {
            ...
            _ => panic!("Backend not implemented: Only Candle is supported for this operation"),
        }
    }
    ...
    //TODO make a nicer return for this
    ...
    pub fn forward(&mut self, options: &PySmplOptions, betas: &PyBetas, pose: &PyPose, expression: Option<&PyExpression>) -> PySmplOutput {
        // let smpl_output = self.get_for_candle_mut().inner.forward(
        //     &betas.inner,
        //     &pose.inner,
