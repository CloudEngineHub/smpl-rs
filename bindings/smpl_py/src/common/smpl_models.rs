...
use pyo3::prelude::*;
use smpl_core::{common::smpl_model::SmplCache, smpl_x::smpl_x_gpu::SmplXGPU};
#[pyclass(name = "SmplCache", unsendable)]
pub struct PySmplModels {
    inner: Option<SmplCache>,
}
#[pymethods]
impl PySmplModels {
    #[staticmethod]
    ...
    pub fn get_model(&mut self, py: Python<'_>, smpl_type: PySmplType, gender: PyGender) -> Py<PyAny> {
        ...
        let ref_smpl_dyn = self.inner.as_ref().unwrap().get_model_box_ref(smpl_type.into(), gender.into()).unwrap();
        let box_smpl = (**ref_smpl_dyn).clone_dyn();
        match box_smpl.as_any() {
            smpl if smpl.downcast_ref::<SmplXGPU>().is_some() => {
                let smplx = smpl.downcast_ref::<SmplXGPU>().unwrap();
                let pysmplx = PySmplX { inner: smplx.clone() };
    ...
    pub fn get(entity_bits: u64, scene_ptr_idx: u64) -> Self {
        ...
        let scene_ptr = scene_ptr_idx as *mut Scene;
        let scene: &mut Scene = unsafe { &mut *scene_ptr };
        let comp = scene.get_comp::<&mut SmplCache>(&entity).unwrap();
        Self { inner: Some(comp.clone()) }
    }
}
