"""Generate type stubs for smpl_rs"""
import os
...
def generate_stubs():
    """Generate all stubs."""
    os.makedirs('smpl_rs', exist_ok=True)
    ...
    module_imports = {
        '__init__': ["from smpl_rs.models import SmplX",
                     "from smpl_rs.types import SmplType, Gender"],
        'types': [],
        'models': ["from smpl_rs.types import SmplType, Gender",
                   "from smpl_rs.components import SmplOptions, Betas, Expression, Pose"],
        'components': ["from smpl_rs.types import SmplType, UpAxis, AngleType, AnimWrap"],
        'builders': [],
        'plugins': [],
        'codec': ["from smpl_rs.types import GltfCompatibilityMode",
                  "from smpl_rs.builders import EntityBuilderSmplRs"]
    }

    for submod in ['types', 'codec', 'models', 'components', 'builders', 'plugins', '__init__']:
        try:
            with open(f'smpl_rs/{submod}.pyi', 'w') as f:
                f.write('\n'.join(common_imports))
                if submod in module_imports:
                    f.write('\n'.join(module_imports[submod]) + '\n\n')
                if submod == '__init__':
                    generate_stub(smpl_rs, f)
                else:
                    generate_stub(getattr(smpl_rs, submod), f, is_types_module=(submod == 'types'))
