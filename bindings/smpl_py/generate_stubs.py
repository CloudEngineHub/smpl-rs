...
def generate_stub(module, f, is_types_module=False):
    ...
    for name, obj in inspect.getmembers(module):
        if inspect.isclass(obj):
            ...
            if not methods and not any(
                m
                for m, _ in inspect.getmembers(obj)
                if callable(getattr(obj, m, None)) and not m.startswith("__")
            ):
                f.write("    pass\n")
            else:
                for method_name, method in inspect.getmembers(obj):
                    ...
                    if sig:
                        ...
                        param_list.extend(
                            f"{k}: {v}" for k, v in params.items() if k != "self"
                        )
                        ret_type = "None"
                        if "->" in sig:
                            ret_type = sig.split("->")[1].strip()
                        f.write(
                            f"    def {method_name}({', '.join(param_list)}) -> {ret_type}: ...\n"
                        )
                    else:
                        f.write(
                            f"    def {method_name}(*args: Any, **kwargs: Any) -> Any: ...\n"
                        )
            f.write("\n")
        ...
        try:
            with open(f"smpl_rs/{submod}.pyi", "w") as f:
                ...
                if submod == "__init__":
                    ...
                    generate_stub(
                        getattr(smpl_rs, submod), f, is_types_module=(submod == "types")
                    )
