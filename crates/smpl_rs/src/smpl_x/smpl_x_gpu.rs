use crate::{
    common::{
        ...
        smpl_options::SmplOptions, types::{Gender, SmplType, UpAxis},
    },
...
impl<B: Backend> SmplXGPU<B> {
    ...
    pub fn new_from_matrices(
        ...
        let b_pose_dirs = pose_dirs
            .map(|pose_dirs| {
                let pose_dirs = pose_dirs
                    .into_shape_with_order((NUM_VERTS * 3, NUM_JOINTS * 9))
                    .unwrap();
        ...
        >::from_floats(lbs_weights_split.as_slice().unwrap(), &device)
            .reshape([idx_vuv_2_vnouv_vec.len(), NUM_JOINTS + 1]);
        ...
        let lbs_weights_split_nd = lbs_weights_nd
            .to_owned()
            .gather(&idx_vuv_2_vnouv_vec, &cols)
            .into();
    ...
    fn new_from_npz_reader<R: Read + Seek>(
        ...
        let parent_idx_per_joint: nd::Array2<i32> = npz
    ...
    B::QuantizedTensorPrimitive<2>: std::marker::Sync,
{
    ...
    fn apply_pose(
        ...
        let nd_joints = nd::Array2::from_shape_vec((shape[0], shape[1]), joints_data)
            .expect("Shape mismatch during tensor to ndarray conversion");
        let parent_idx_data_i32: Vec<i32> = tensor_to_data_int(
        ...
        let parent_idx_data_u32: Vec<u32> = parent_idx_data_i32
            ...
            .map(|x| x as u32)
            .collect();
        let (posed_joints_nd, rel_transforms_nd) = batch_rigid_transform(
            parent_idx_data_u32,
            &rot_mats,
            &nd_joints,
            NUM_JOINTS,
        ...
        let W = lbs_weights;
        let A_nd = rel_transforms_nd
            .into_shape_with_order((NUM_JOINTS + 1, 16))
            .unwrap();
        ...
        let verts_final = Tensor::<
            B,
            1,
        >::stack(
            vec![
                verts_final_0.squeeze(1), verts_final_1.squeeze(1), verts_final_2
                .squeeze(1)
            ],
            1,
        );
        ...
        let mut normals_final = if let Some(normals) = normals {
            ...
            let normals_final = Tensor::<
                B,
                1,
            >::stack(
                vec![normals_0.squeeze(1), normals_1.squeeze(1), normals_2.squeeze(1)],
                1,
            );
            Some(normals_final)
        } else {
        ...
        let mut tangents_final = if let Some(tangents) = tangents {
            ...
            let handedness: Tensor<B, 1> = tangents
                .clone()
                .slice([0..nr_verts, 3..4])
                .squeeze(1);
            let tangents_final = Tensor::<
                B,
                1,
            >::stack(
                vec![
                    tangents_0.squeeze(1), tangents_1.squeeze(1), tangents_2.squeeze(1),
                    handedness
                ],
                1,
            );
        ...
        let trans_pose_broadcasted_v = trans_pose
            .clone()
            .reshape([1, 3])
            .expand(verts_final.shape());
        let trans_pose_broadcasted_p = trans_pose
            .reshape([1, 3])
            .expand(posed_joints.shape());
        let mut verts_final_modified = verts_final
            .clone()
            .add(trans_pose_broadcasted_v.clone());
        let mut posed_joints_modified = posed_joints
            .clone()
            .add(trans_pose_broadcasted_p.clone());
        if pose.up_axis == UpAxis::Z {
            let vcol0: Tensor<B, 1> = verts_final_modified
                .clone()
                .slice([0..nr_verts, 0..1])
                .squeeze(1);
            let vcol1: Tensor<B, 1> = verts_final_modified
                .clone()
                .slice([0..nr_verts, 1..2])
                .squeeze(1);
            let vcol2: Tensor<B, 1> = verts_final_modified
                .clone()
                .slice([0..nr_verts, 2..3])
                .squeeze(1);
            let verts_new_col1 = vcol2;
            let verts_new_col2 = vcol1.mul_scalar(-1.0);
            verts_final_modified = Tensor::stack::<
                2,
            >(vec![vcol0, verts_new_col1, verts_new_col2], 1);
            let jcol0: Tensor<B, 1> = posed_joints_modified
                .clone()
                .slice([0..nr_joints, 0..1])
                .squeeze(1);
            let jcol1: Tensor<B, 1> = posed_joints_modified
                .clone()
                .slice([0..nr_joints, 1..2])
                .squeeze(1);
            let jcol2: Tensor<B, 1> = posed_joints_modified
                .clone()
                .slice([0..nr_joints, 2..3])
                .squeeze(1);
            let joints_new_col1 = jcol2;
            let joints_new_col2 = jcol1.mul_scalar(-1.0);
            posed_joints_modified = Tensor::stack::<
                2,
            >(vec![jcol0, joints_new_col1, joints_new_col2], 1);
            if let Some(ref mut normals) = normals_final {
                let ncol0: Tensor<B, 1> = normals
                    .clone()
                    .slice([0..nr_verts, 0..1])
                    .squeeze(1);
                let ncol1: Tensor<B, 1> = normals
                    .clone()
                    .slice([0..nr_verts, 1..2])
                    .squeeze(1);
                let ncol2: Tensor<B, 1> = normals
                    .clone()
                    .slice([0..nr_verts, 2..3])
                    .squeeze(1);
                let normals_new_col1 = ncol2;
                let normals_new_col2 = ncol1.mul_scalar(-1.0);
                let normals_final_modified = Tensor::stack::<
                    2,
                >(vec![ncol0, normals_new_col1, normals_new_col2], 1);
                *normals = normals_final_modified;
            }
            if let Some(ref mut tangents) = tangents_final {
                let tcol0: Tensor<B, 1> = tangents
                    .clone()
                    .slice([0..nr_verts, 0..1])
                    .squeeze(1);
                let tcol1: Tensor<B, 1> = tangents
                    .clone()
                    .slice([0..nr_verts, 1..2])
                    .squeeze(1);
                let tcol2: Tensor<B, 1> = tangents
                    .clone()
                    .slice([0..nr_verts, 2..3])
                    .squeeze(1);
                ...
                let tangents_final_modified = Tensor::stack::<
                    2,
                >(vec![tcol0, tangents_new_col1, tangents_new_col2, handedness], 1);
                *tangents = tangents_final_modified;
            }
        }
        (
            verts_final_modified,
            normals_final.clone(),
            tangents_final.clone(),
            posed_joints_modified,
        )
    }
