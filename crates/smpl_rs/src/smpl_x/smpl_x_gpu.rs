...
use utils_rs::bshare::{tensor_to_data_float, tensor_to_data_int, ToBurn};
...
// https://docs.rs/burn/0.13.2/burn/backend/type.Wgpu.html
// Note from the docs for WASM -----------
// This version of the wgpu backend uses burn_fusion to compile and optimize
// streams of tensor operations for improved performance. You can disable the
// fusion feature flag to remove that functionality, which might be necessary on
// wasm for now.
...
pub struct SmplXGPU<B: Backend> {
    ...
    // Tensors that will be allocated on the GPU with the `burn` backend
    pub verts_template: Tensor<B, 2, Float>, // Vertices template
    pub faces: Tensor<B, 2, Int>,            // Faces tensor
    pub faces_uv_mesh: Tensor<B, 2, Int>,    // UV mesh faces
    pub uv: Tensor<B, 2, Float>,             // UV coordinates
    pub shape_dirs: Tensor<B, 2, Float>,     // Shape directions
    pub expression_dirs: Option<Tensor<B, 2, Float>>,
    pub pose_dirs: Option<Tensor<B, 2, Float>>,  // Pose directions (optional)
    pub joint_regressor: Tensor<B, 2, Float>,    // Joint regressor
    pub parent_idx_per_joint: Tensor<B, 1, Int>, // Parent indices per joint
    pub lbs_weights: Tensor<B, 2, Float>,        // Linear blend skinning weights
    pub verts_ones: Tensor<B, 2, Float>,         // Vertices with ones appended
    pub idx_vuv_2_vnouv: Tensor<B, 1, Int>,      // Index mapping between UV and non-UV vertices

    // Stored as nalgebra matrices for specific operations
    pub faces_na: na::DMatrix<u32>,         // Faces matrix
    pub faces_uv_mesh_na: na::DMatrix<u32>, // UV mesh faces matrix
    pub uv_na: na::DMatrix<f32>,            // UV coordinates matrix
    pub idx_vuv_2_vnouv_vec: Vec<usize>,    // Index vector mapping between UV and non-UV vertices
    pub lbs_weights_split: Tensor<B, 2>,    // Split linear blend skinning weights

    // Store the converted arrays in the struct // Change this
...
impl<B: Backend> SmplXGPU<B> {
    ...
    pub fn new_from_matrices(
        ...
        // Process shape_dirs and pose_dirs
        ...
        // Handle idx_vuv_2_vnouv and faces_na
        ...
        // let mut idx_vuv_2_vnouv = nd::ArcArray1::<i32>::zeros(max_v_uv_idx as usize +
        // 1);
        ...
        for (fuv, fnouv) in ft.axis_iter(nd::Axis(0)).zip(faces.axis_iter(nd::Axis(0))) {
            //3 indices into vt(vector with uv coordinates)
            let uv_0 = fuv[[0]];
            let uv_1 = fuv[[1]];
            let uv_2 = fuv[[2]];
            //3 indices into verts_template
            let nouv_0 = fnouv[[0]];
            let nouv_1 = fnouv[[1]];
            let nouv_2 = fnouv[[2]];
        ...
        // Convert idx_vuv_2_vnouv from ndarray to Vec<i32>
        let idx_vuv_2_vnouv_vec: Vec<i32> = idx_vuv_2_vnouv.mapv(|x| x).into_raw_vec_and_offset().0; // Convert ndarray into Vec<i32>

        // Convert Vec<i32> to a slice
        let idx_vuv_2_vnouv_slice: &[i32] = &idx_vuv_2_vnouv_vec;

        // Create the burn tensor with integer (Int) kind
        let b_idx_vuv_2_vnouv = Tensor::<B, 1, Int>::from_ints(idx_vuv_2_vnouv_slice, &device);
        // Convert idx_vuv_2_vnouv from ndarray to burn tensor using the helper function
        ...
        // let faces_uv_mesh_na = ft.view().into_nalgebra().clone_owned().map(|x| x as
        // u32);
        ...
        // Create the struct with all the converted fields
        Self {
    ...
    fn new_from_npz_reader<R: Read + Seek>(
        ...
        // let shape_dirs: nd::Array3<f32> = npz.by_name("shape_dirs.npy").unwrap();
        // let pose_dirs: Option<nd::Array3<f32>> = npz.by_name("pose_dirs.npy").ok();
        // //pose_dirs is optional since we might load it in a deferred way
        let full_shape_dirs: nd::Array3<f32> = npz.by_name("shapedirs").unwrap();
        //the full_shape_dirs might contain both the actual shape_dirs (300betas) and
        // the expression_dirs(100expression_components) which is the usual case when
        // loading the model from the mpi website. However it's very confusing
        // to have them both in the same matrix so if there exists a expression_dirs.npy
        // I will assume that that's the expression_dirs and the shape_dirs just
        // corresponds to the betas let (shape_dirs, expression_dirs) = if npz.
        let (shape_dirs, expression_dirs) = if let Ok(expression_dirs) = npz.by_name("expressiondirs") {
            (full_shape_dirs, Some(expression_dirs))
        } else {
            //there is not expression_dirs so shape_dirs actually contains both shape and
            // expression but we still check if we can actually slide the 300betas and the
            // 100expression params from it
            let shape_dirs = full_shape_dirs
                .slice_axis(nd::Axis(2), nd::Slice::from(0..max_num_betas.min(300)))
                .to_owned();

            //double check that we can actually slice 100 expressions
        ...
        // let pose_dirs: nd::Array3<f32> = npz.by_name("pose_dirs").unwrap();
        let pose_dirs: Option<nd::Array3<f32>> = npz.by_name("posedirs").ok(); //pose_dirs is optional since we might load it in a deferred way
        ...
        // println!("Original shape of parent_idx_per_joint: {:?}",
        // parent_idx_per_joint.shape());
        ...
        // println!("parent_idx_per_joint {:?}", parent_idx_per_joint.to_vec());
...
where
    B::FloatTensorPrimitive<2>: Sync, // Ensure that FloatTensorPrimitive<2> implements Sync
    B::IntTensorPrimitive<2>: Sync,   // Also ensure the integer tensors implement Sync
    ...
    B::QuantizedTensorPrimitive<2>: std::marker::Sync,
{
    ...
    fn forward(&self, options: &SmplOptions, betas: &Betas, pose_raw: &Pose, expression: Option<&Expression>) -> SmplOutputDynamic<B> {
        ...
        let (verts_posed_nd, _, _, joints_posed) = self.apply_pose(
            &verts_t_pose,
            None, //TODO make this optional
            None,
            &joints_t_pose,
            &self.lbs_weights,
            &pose,
    ...
    fn betas2verts(&self, betas: &Betas) -> Tensor<B, 2, Float> {
        ...
        let betas_slice = betas.betas.as_slice().unwrap(); // Convert ndarray to a slice
    ...
    // #[allow(non_snake_case)]
    // #[allow(clippy::let_and_return)]
    // fn expression2offsets(&self, expression: &Expression) -> nd::Array2<f32> {
    ...
    //         let expression_dirs = expression_dirs.slice(s![..,
    // 0..input_nr_expression_coeffs]);

    //         //dot an add
    //         let v_expr_offsets = expression_dirs.dot(&expression.expr_coeffs);
    //         v_expr_offsets.into_shape_with_order((NUM_VERTS, 3)).unwrap()
    //     } else {
    //         nd::Array2::<f32>::zeros((NUM_VERTS, 3))
    //     };
    ...
    fn expression2offsets(&self, expression: &Expression) -> Tensor<B, 2, Float> {
        ...
        let offsets = if let Some(ref expression_dirs) = self.expression_dirs {
            ...
            // Slice the expression_dirs tensor for the input number of expression
            // coefficients
            let expression_dirs_sliced = expression_dirs
                .clone()
                .slice([0..expression_dirs.dims()[0], 0..input_nr_expression_coeffs]);

            // Convert the expression coefficients ndarray to a Dynamic tensor
            let expr_coeffs_tensor = Tensor::<B, 1, Float>::from_floats(expression.expr_coeffs.as_slice().unwrap(), &device);

            // Matrix multiplication (dot product equivalent) between expression_dirs and
            // expr_coeffs_tensor
            ...
            // If no expression_dirs, return a tensor of zeros with shape [NUM_VERTS, 3]
    ...
    fn compute_pose_correctives(&self, pose: &Pose) -> Tensor<B, 2, Float> {
        ...
        let offsets = if let Some(pose_dirs) = &self.pose_dirs {
            // println!("Smplx pose correctives fn 0 takes {:?}", now.elapsed());
            ...
            let mut rot_mats = batch_rodrigues(full_pose); //from  (NUM_joints+1,3) to (num_joints+1,3,3)
                                                           // println!("Smplx pose correctives fn 1 takes {:?}", now.elapsed());
            ...
            // println!("Smplx pose correctives fn 2 takes {:?}", now.elapsed());

            let new_pose_dirs = pose_dirs.clone();
            // println!("Smplx pose correctives fn 2.1 takes {:?}", now.elapsed());
            // The matmul below is almost all the time taken by this function
            let all_pose_offsets = new_pose_dirs.matmul(b_pose_feature);
            // println!("Smplx pose correctives fn 2.2 takes {:?}", now.elapsed());
            all_pose_offsets.reshape([NUM_VERTS, 3])

            // (NUM_VERTICES*3, NUM_JOINTS*9) x (NUM_JOINTS*9) ->
            // (NUM_VERTICES*3) pose_dirs
            //     .clone()
            //     .matmul(b_pose_feature)
            //     .reshape([NUM_VERTS, 3])
        } else {
            Tensor::<B, 2, Float>::zeros([NUM_VERTS, 3], &self.device)
        };
        // println!("Smplx pose correctives fn 3 takes {:?}", now.elapsed());
        offsets
    }
    ...
    fn apply_pose(
        ...
        lbs_weights: &Tensor<B, 2, Float>, // Now using burn Tensor for LBS weights
        pose: &Pose,
    ) -> (
        Tensor<B, 2, Float>,         // v
        ...
        let shape = joints.shape().dims; // Get the shape of the tensor
        ...
        // let posed_joints = ndarray_to_burn_tensor_2d(
        //     &posed_joints_nd,
        //     [posed_joints_nd.shape()[0], posed_joints_nd.shape()[1]],
        //     &self.device,
        // );
        ...
        let v_posed = verts_t_pose.clone(); // Placeholder: If any processing is required here, perform it

        let W = lbs_weights; // V x (J + 1)
        let A_nd = rel_transforms_nd.into_shape_with_order((NUM_JOINTS + 1, 16)).unwrap();
        // let A = ndarray_to_burn_tensor_2d(&A_nd, [NUM_JOINTS + 1, 16], &self.device);
        let A = A_nd.to_burn(&self.device);

        let T = W.clone().matmul(A).reshape([nr_verts, 4, 4]);

        let dims_3 = 3; // Third dimension size

        // Extract rotations and translations from T using squeeze to reduce
        // dimensionality
        ...
        // Multiply rot0, rot1, rot2 with v_posed and sum along axis 1 to mimic the
        // sum_axis logic from ndarray
        ...
        // Add translation to verts_final
        let verts_final = verts_final.add(trans);

        // Compute transformed normals if they exist
        ...
        // Compute transformed tangents if they exist
        let mut tangents_final = if let Some(tangents) = tangents {
            // Tangent vector without the w (handedness) component, shape (nr_verts, 3)
            let tangents_3 = tangents.clone().slice([0..nr_verts, 0..3]);

            // Perform matrix multiplication for the first 3 components
            ...
            // Extract the handedness (w component), shape (nr_verts, 1)
            let handedness: Tensor<B, 1> = tangents.clone().slice([0..nr_verts, 3..4]).squeeze(1);

            // Stack the transformed tangent vector (x, y, z) and the handedness (w) back
            // together
            let tangents_final = Tensor::<B, 1>::stack(
                vec![
                    tangents_0.squeeze(1),
                    tangents_1.squeeze(1),
                    tangents_2.squeeze(1),
                    handedness, // Keep handedness unchanged
                ],
                1,
            );
        ...
        // let trans_pose = nd_burn_share::ndarray_to_burn_tensor_1d(&trans_pose_nd,
        // &self.device);
        ...
        if pose.up_axis == UpAxis::Z {
            ...
            let vcol1: Tensor<B, 1> = verts_final_modified.clone().slice([0..nr_verts, 1..2]).squeeze(1); // Y-axis column
            let vcol2: Tensor<B, 1> = verts_final_modified.clone().slice([0..nr_verts, 2..3]).squeeze(1); // Z-axis column

            // Swap Y and Z columns and invert the Z values (vcol1 becomes the new Z, vcol2
            // becomes the new Y)
            let verts_new_col1 = vcol2; // Z becomes new Y
            let verts_new_col2 = vcol1.mul_scalar(-1.0); // Y becomes new Z, with sign inversion

            // Reconstruct the final tensor by stacking the modified columns back together
            verts_final_modified = Tensor::stack::<2>(
                vec![
                    vcol0,          // First column (X-axis)
                    verts_new_col1, // Swapped Y column
                    verts_new_col2, // Swapped and inverted Z column
                ],
                1,
            );

            // Similar operations for posed_joints
            ...
            if let Some(ref mut tangents) = tangents_final {
                ...
                // Extract the handedness (fourth component)
                let handedness = tangents.clone().slice([0..nr_verts, 3..4]).squeeze(1);

                // Stack the transformed tangent vector (x, y, z) and preserve the handedness
                // (w)
                let tangents_final_modified = Tensor::stack::<2>(
                    vec![
                        tcol0,             // X component
                        tangents_new_col1, // Transformed Y component
                        tangents_new_col2, // Transformed Z component
                        handedness,        // Preserve handedness (w)
                    ],
                    1,
                );
