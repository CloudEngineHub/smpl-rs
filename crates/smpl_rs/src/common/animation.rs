...
pub struct AnimationRunner {
    pub anim_current_time: Duration, //current timing within this animation
    pub anim_reversed: bool,
    pub nr_repetitions: u32, // nr of times the animation looped
    pub paused: bool,        // can be set by the gui with a button to be true or false
    pub temporary_pause: bool, /* can be set to true while dragging a slider. Needs to be separate than the paused flag because when the slider is
                              * not dragged the temporary pause is set to false but the pause flag can still be true */
}
...
pub struct Animation {
    pub per_frame_joint_poses: nd::Array3<f32>, /* (num_animation_frames, num_joints, 3) containing axis angle for each joint for every frame of
                                                 * the animation */
    pub per_frame_root_trans: nd::Array2<f32>, // (num_animation_frames, 3) containing root translation for each frame of the animation
    pub per_frame_expression_coeffs: Option<nd::Array2<f32>>, // (num_animation_frames, 10-100)
    pub start_offset: usize,                   // For multi-person animations we want to offset for visibility
    pub runner: AnimationRunner,
    pub config: AnimationConfig,
}
// dont use this because it will get called whenever we clone an anim and then
// almost immediatelly drop it Ideally the callback would be under a Rc so that
// it only triggers when the number of strong references drops to 1
// therefore multiple clones of the animation would not cause multiple
// callbacks. however we would also like the callback to receive the scene as
// parameter so that we do something interesting, this is too complicated so we
// drop it for now impl Drop for Animation {
//     fn drop(&mut self) {
//         if let Some(cb) = self.cb_finish {
//             log!(
//                 "dropping animation and calling cb with {}",
//                 self.is_finished()
//             );
//             cb(self.is_finished()); //return true to the callback if the
...
impl Animation {
    ...
    pub fn new_from_matrices(
        ...
        // transform from euler to axis angle
        if config.smpl_type != SmplType::SmplPP && config.angle_type == AngleType::Euler {
            let animation_frames = per_frame_joint_poses.dim().0;
            let num_active_joints = per_frame_joint_poses.dim().1;

            let mut new_per_frame_joint_poses: nd::Array3<f32> = nd::Array3::<f32>::zeros((animation_frames, num_active_joints, 3)); //(num_animation_frames, num_joints_active*3)
            ...
            for (idx_timestep, poses_for_timestep) in per_frame_joint_poses.axis_iter(nd::Axis(0)).enumerate() {
                for (idx_joint, joint_pose) in poses_for_timestep.axis_iter(nd::Axis(0)).enumerate() {
                    // for some reason the math needs to follow the threejs one instead of the
                    // nalgebra one so we do it like this
    ...
    fn new_from_npz_reader<R: Read + Seek>(npz: &mut NpzReader<R>, config: AnimationConfig) -> Self {
        ...
        let per_frame_joint_poses: nd::Array2<f64> = npz.by_name("poses").unwrap(); //(num_animation_frames, num_joints_active*3)
    ...
    pub fn new_from_json(path: &str, _anim_fps: f32, config: AnimationConfig) -> Self {
        ...
        // assumes the json data has shape (animation_frames, num_active_joints, 3) and
        // just dumps it directly into the ndarray
    ...
    pub fn new_from_smpl_codec(codec: &SmplCodec, wrap_behaviour: AnimWrap) -> Option<Self> {
        ...
        // per_frame_joint_poses (num_animation_frames, num_joints_active, 3)
        ...
        if codec.smpl_type() == SmplType::SmplPP {
            let body_pose = codec
                ...
                .clone(); //(num_animation_frames, nr_joints,3)
            ...
            .unwrap();
            let per_frame_expression_coeffs = codec.expression_parameters.clone(); //(10-100)
    ...
    pub fn advance(&mut self, dt_raw: Duration, first_time: bool) {
        ...
        // Wrap behaviour should be handled by the .advance of SceneRunner now,
        // This should simply progress
    ...
    pub fn slice_time_range(&self, start_sec: f32, end_sec: f32) -> Animation {
        // Make a clone of the current anim because we need to move some times withing
        // it and I really don't want to make self mutable
        let mut cur_anim = self.clone();

        // Advance to the start and end point and get the idx of the start and end frame
        ...
        // Make a new matrices with timeframes from start_idx to end_idx inclusive
        ...
        // Get also expression if we have any
    ...
    pub fn align_y_axis_quadrant(&mut self, current_axis: &nd::Array1<f32>, desired_axis: &nd::Array1<f32>) {
        // We only care about the xz component because that is aligned with the floor
        // do it nalgebra because ndarray_linalg crate doesn't work very nicely on wasm
        // the -y is required becase a 3d vector that goes "into" the screen  like [0,0,
        // -1.0] should actually map to the 2D vector [0.0,1.0] because x goes to the
        // right and y goes up now
        ...
        // Do increments of 90 degrees and check which one brings it more in alignment
        // with the desired axis
        ...
        for _iters in 0..4 {
            ...
            // Check the current dot product and if it's closer to 1.0, then replace
        ...
        // Make a rotation matrix along the Y axis the corresponds to the best_angle and
        // apply it to the pelvis
