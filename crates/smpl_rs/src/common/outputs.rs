...
pub struct SmplOutputDynamic<B: Backend> {
    ...
    pub normals: Option<Tensor<B, 2, Float>>, // Optional because normals aren't always computed
    pub uvs: Option<Tensor<B, 2, Float>>,     // Optional because UVs are only needed when vertices are split
    pub joints: Tensor<B, 2, Float>,
}
